...
import com.engie.csai.pc.pbftSimulatorold2.replica.ByztReplica;
...
public class PBFTsimulator
{
    ...
    //public static int numberOfClients = 3; // CN -> numberOfClients						 //Number of clients
    /**
     ...
     * <p>
     * We tested this simulator with 5 replicas (processor peers) and 30 clients. It did not face a congestion.
...
//    public static final int requestNumber = 5; // REQNUM -> RequestNumbers
     ...
     * Paxos and Raft can reach to 150,000 - 200,000 tps.
     *
    ...
    /**
     * REQNUM determines number of client requests.
     * It is set to 5000 by default.
     * In general, it should be equal to the number of clients' requests sent to each committee.
    ...
    public int[][] netDlysToClis;

    public int[][] netDlysToNodes;

    private final Integer nbClients;
    private final Integer nbPeers;
    private final Integer requestNumber;

    public PBFTsimulator(Integer nbClients, Integer nbPeers, Integer nbRequests){
        this.nbClients = nbClients;
        this.nbPeers = nbPeers;
        this.requestNumber = nbRequests;
        netDlys = netDlyBtwRpInit(nbPeers);
        netDlysToClis = netDlyBtwRpAndCliInit(nbPeers, nbClients);
        netDlysToNodes = Utils.flipMatrix(netDlysToClis);
    }
    ...
    public void launch(String messageClientRequest)
    {
        ...
        boolean[] byzantines = byztDistriInit(PBFTsimulator.this.nbPeers, numberOfFaultyNodes); // byzts -> byzantines
        // boolean[] byzantines = {true, false, false, false, false, false, true};
        Replica[] replicas = new Replica[PBFTsimulator.this.nbPeers]; // reps -> replicas
        for (int i = 0; i < PBFTsimulator.this.nbPeers; i++)
        {
            if (byzantines[i])
            {
                /**
        ...
        Client[] clients = new Client[nbClients]; // clis -> clients
        ...
        {
            clients[rand.nextInt(nbClients)].sendRequest(this, 0, messageClientRequest);
            requestNums++;
        }
        ...
        while (!msgQue.isEmpty())
        {
            ...
            {
                clients[rand.nextInt(nbClients)].sendRequest(this, msg.rcvtime, messageClientRequest);
                requestNums++;
            }
            inFlyMsgLen -= msg.len;
        ...
        double tps = getStableRequestNum(clients) /  (timestamp / 1000d);
        System.out.println("【The end】 Category : "+messageClientRequest+ " -- The average message confirmation time: " + totalTime / totalStableMsg
                + " millisecond. The message throughput: " + tps + " tps\n timestamp = " + timestamp +"\n stableRequestNum " + getStableRequestNum(clients));
    }
