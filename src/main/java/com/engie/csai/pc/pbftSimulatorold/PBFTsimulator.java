...
import com.engie.csai.pc.pbftSimulatorold.replica.ByztReplica;
import com.engie.csai.pc.pbftSimulatorold.replica.Replica;
...
public class PBFTsimulator {
	...
	private int[][] netDlys;
	private int[][] netDlysToClis;
	private int[][] netDlysToNodes ;

	public int[][] getNetDlys()
	{
		return netDlys;
	}

	public int[][] getNetDlysToClis()
	{
		return netDlysToClis;
	}

	public int[][] getNetDlysToNodes()
	{
		return netDlysToNodes;
	}
	...
	public int getNumberOfFaultyNodes()
	{
		return numberOfFaultyNodes;
	}
	...
	private final int numberOfNodes; // RN -> numberOfNodes  						//replicas节点的数量(rn) //Number of nodes

	private final int numberOfFaultyNodes ; // FN -> numberOfFaultyNodes						//恶意节点的数量 //Number of malicious nodes

	private final int numberOfClients; // CN -> numberOfClients						//客户端数量 //Number of clients
	...
	private final int reqNum;					//请求消息总数量 //Total number of request messages
	 ...
	 * (5 replicas and 3/5 clients: throughput: 1666 tps). (With 12 replicas also we got the same results.)
	 *
	...
	public static void main (String[] args){
		//launch();
		PBFTsimulator sim = new PBFTsimulator(5,0,5, 5000);
		sim.launch();
	}

	public PBFTsimulator(int numberOfNodes, int numberOfFaultyNodes, int numberOfClients, int reqNum){
		netDlys = netDlyBtwRpInit(numberOfNodes);
		netDlysToClis = netDlyBtwRpAndCliInit(numberOfNodes, numberOfClients);
		netDlysToNodes = Utils.flipMatrix(netDlysToClis);
		this.numberOfNodes=numberOfNodes;
		this.numberOfFaultyNodes=numberOfFaultyNodes;
...
//		boolean[] byzantines = {true, false, false, false, false, false, true};
		...
		for(int i = 0; i < Math.min(INFLIGHT, reqNum); i++) {
			clients[rand.nextInt(numberOfClients)].sendRequest(0);
			requestNums++;
		}
...
//		int ttt = 0;
		while(!msgQue.isEmpty()) {
			...
			if(requestNums - getStableRequestNum(clients) < INFLIGHT && requestNums < reqNum) {
				clients[rand.nextInt(numberOfClients)].sendRequest(msg.rcvtime);
				requestNums++;
			}
			inFlyMsgLen -= msg.len;
