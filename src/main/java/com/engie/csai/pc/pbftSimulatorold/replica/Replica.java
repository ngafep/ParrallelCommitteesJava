...
import com.engie.csai.pc.pbftSimulatorold.Client;
import com.engie.csai.pc.pbftSimulatorold.PBFTsimulator;
import com.engie.csai.pc.pbftSimulatorold.Utils;
import com.engie.csai.pc.pbftSimulatorold.message.*;
...
//				System.out.println("send:"+checkptMsg.toString());
	...
	public boolean prepared(PrePrepareMsg m) {
		...
		if(cnt >= 2 * Utils.getMaxTorelentNumber(pbfTsimulator.getNumberOfNodes())) {
			return true;
		}
		return false;
	}
	...
	public void garbageCollect() {
		...
		for(Message msg : checkptMsgSet) {
			...
			if(cnt > Utils.getMaxTorelentNumber(pbfTsimulator.getNumberOfNodes())) {
...
//		System.out.println(id+"[水位]"+h+"-"+(h+L));
	...
	public void receivePreprepare(Message msg) {
		...
		if(msgv < viewNumber || !inWater(msgn) || i != msgv % pbfTsimulator.getNumberOfNodes() || !hasNewView(viewNumber)) {
			return;
		}
	...
	public void receiveTimeOut(Message msg) {
		...
		if(reqStats.size() >= pbfTsimulator.getReqNum()) return;
		isTimeOut = true;
		//发送viewChange消息
	...
	public int getPriId() {
		return viewNumber % pbfTsimulator.getNumberOfNodes();
	}
